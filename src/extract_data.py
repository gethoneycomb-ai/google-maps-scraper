...
def get_can_claim(data):
    ...
    link = safe_get(data, 6, 226, 0)
    if isinstance(link, str):
        path = extract_path_from_link(link)
        if path and path.rstrip("/").endswith("setup/create"):
            return True

    text = safe_get(data, 6, 49, 1)
    if isinstance(text, str) and (
        text.lower().startswith("claim") or " claim" in text.lower()
    ):
        return True
...
def get_description(data):
    return safe_get(data, 6, 154, 0, 0) or safe_get(data, 6, 32, 1, 1)
...
def extract_questions(data):
    ...
    for element in images:
        ...
        question_date = (
            convert_timestamp_to_iso_date(new_var)
            if safe_get(question_data, -1)
            else None
        )
        ...
        answer_date = (
            convert_timestamp_to_iso_date(new_var1)
            if safe_get(answer_data, 0, -1)
            else None
        )
...
def extract_competitors(data, link):
    ...
    sorted_data = sorted(ls, key=competitor_sorting_key, reverse=True)
...
def get_options(data):
    return [
        {"name": x[1], "enabled": (safe_get(x, 2, 1, 0, 0) or x[4][0]) != 0}
        for x in data
    ]
...
def extract_most_popular_times(data):
    if isinstance(data, dict):
        ...
        hour_popularity = {
            hour: {"total_popularity": 0, "count": 0, "time_label": ""}
        ...
        for day, hours in data.items():
            for hour_data in hours:
                ...
                hour_popularity[hour]["total_popularity"] += hour_data[
                    "popularity_percentage"
                ]
                hour_popularity[hour]["count"] += 1
                hour_popularity[hour]["time_label"] = hour_data[
                    "time_label"
                ]  # Storing the time label
        ...
        sorted_avg_popularity = sorted(
            average_popularity, key=lambda x: x["average_popularity"], reverse=True
        )
